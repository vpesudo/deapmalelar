...
def main():
	...
	sumPMTs=data['sumPMTs']
	sumPMTs=sumPMTs[-test_size:]
	print("lengths:", len(sumPMTs), len(testY))
	...
	print(R.shape)
	filename = "./r_error_ANN_epochs%d_2HL_%d_%d_%d_%d_binaryclass.txt" % (nepochs, neurons[0],neurons[1],cutlow,cuthigh)
	fh = open(filename, "w")
	...
	plt.hist(errors, 40)
	plt.title('Error X')
	filename = "./histerrors_ANN_epochs%d_2HL_%d_%d_%d_%d_binaryclass.eps" % (nepochs, neurons[0],neurons[1],cutlow,cuthigh)
	...
	filename = "./loss_ANN_epochs%d_2HL_%d_%d_binaryclass.eps" % (nepochs, neurons[0],neurons[1])
	#plt.savefig(filename)
	...
	plt.ylim(-1.1,1.1)
	plt.legend(['train', 'test'], loc='upper left')
	filename = "./scatterplot_ANN_epochs%d_2HL_%d_%d_%d_%d_binaryclass.eps" % (nepochs, neurons[0],neurons[1],cutlow,cuthigh)
	#plt.savefig(filename)
	...
	filename = "./scatterplot_TruevsPred_ANN_epochs%d_2HL_%d_%d_%d_%d_binaryclass.eps" % (nepochs, neurons[0],neurons[1],cutlow,cuthigh)


	fig,ax = plt.subplots(nrows=1,ncols=1,figsize=(8,8))# 6,6
	plt.figure(5)
	plt.scatter(data['sumPMTs'], errors, s=0.1, color='black')
	plt.scatter(data['sumPMTs'], testY, s=0.1, color='red')
	#plt.title('model loss')
	plt.ylabel('error')
	plt.xlabel('charge')
	plt.ylim(-1.1,1.1)
